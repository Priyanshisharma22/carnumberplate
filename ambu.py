...
# --------------------------------------------------
# DRAW RED ROUTE (NO ANT-PATH duplicates)
# --------------------------------------------------
...
# --------------------------------------------------
# MOVING AMBULANCE — ONLY ONE INSTANCE
# --------------------------------------------------
move_js = f"""
<script>
...
function initAmb() {{
    if (ambMarker === null) {{
        ambMarker = L.marker(route[0], {{icon: ambIcon}}).addTo({m.get_name()});
        moveAmb();
    }}
}}
...
function moveAmb() {{
    ...
    ambMarker.setLatLng(route[idx]);
    idx += 1;

    setTimeout(moveAmb, 700);
}}

document.addEventListener("DOMContentLoaded", initAmb);
